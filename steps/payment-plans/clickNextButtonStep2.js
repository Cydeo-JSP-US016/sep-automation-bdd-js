...
Then('the review step stepper circle is blue', async function () {
    await expect(startApplicationPage.reviewStepCircle).toHaveCSS("background-color", "rgb(1, 201, 255)"); // blue color
});

Then('the payment step stepper circle is green', async function () {
    await expect(startApplicationPage.paymentPlanStepCircle).toHaveCSS("background-color", "rgb(172, 245, 138)"); // green color
});

Then('the start application step stepper circle is green', async function () {
    await expect(startApplicationPage.startApplicationStepCircle).toHaveCSS("background-color", "rgb(172, 245, 138)"); // green color
});
...
Then('user should be navigated back to start application step', async function () {
    await expect(startApplicationPage.startApplicationStepCircle).toHaveCSS("background-color", "rgb(1, 201, 255)"); // blue
});